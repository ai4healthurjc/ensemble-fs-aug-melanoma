...
''' Para añadir random state probablemente haya que cambiar np.random.choice por 
    randint.
...
class MRMR(FSMethod):
    ...
    def fit(self,
        ...
        list_selected_features_index = [df_features.columns.get_loc(var_name) for var_name in
                                        v_selected_feature_names_sorted]
...
class Relieved_F(FSMethod):
    ...
    def fit(self,
        ...
        df_feature_scores = pd.DataFrame(np.zeros((len(list_selected_features_indices), 2)),
                                         columns=['var_name', 'score'])
...
# @track_emissions # writes the emissions to a csv at the current directory.
...
def compute_zmatrix_bootstrap(df_features: pd.DataFrame,
    ...
    """
    Computes the bootstrap and the ensembles, both voting and mean approaches.
    Parameters
    ----------
    df_features : pd.DataFrame
    ...
    M : int, optional
        Number of partitions to be used on the bootstrap. The default is 50.
    ...
    list_dict_fs_result = Parallel(n_jobs=n_jobs)(
        delayed(train_fs_method_by_bootstrap)(dict_boot_data, fs_method_name, v_feature_names, list_vars_categorical,
                                              list_vars_numerical) for dict_boot_data in list_dict_boot_data
    )
...
def train_fs_method_by_bootstrap(dict_boot_data, fs_method_name, v_feature_names, list_vars_categorical,
                                 list_vars_numerical):
...
def identify_type_features(df, discrete_threshold=10, debug=False):
    ...
    if debug:
        print(
            f'Counted {sum(feature_info["type"] == "d")} discrete features and {sum(feature_info["type"] == "c")} continuous features')
